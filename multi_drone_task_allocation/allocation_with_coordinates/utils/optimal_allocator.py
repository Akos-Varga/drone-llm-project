...
def build_mathematical_scheduler(subtasks_with_drones, travel_times, time_limit_s=5):
    """
    ...
    task_name_to_idx = {t[0]: i for i, t in enumerate(tasks)}
    ...
    for task_i, (*_, allowed) in enumerate(tasks):
        ...
        node_index = routing.NodeToIndex(task_i)
    ...
    for v, drone in enumerate(drones):
        ...
        while not routing.IsEnd(idx):
            ...
            # Arrival at 'next_idx' in scaled units:
            arrival_next = solution.Value(time_dim.CumulVar(next_idx)) / time_scale
...
if __name__ == "__main__":
    ...
    schedule = build_mathematical_scheduler(subtasks_with_drones, travel_times, time_limit_s=3)
